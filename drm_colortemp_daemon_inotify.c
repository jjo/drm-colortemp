...
// Global context instance (still global, but now encapsulated)
daemon_ctx_t ctx = {.running = 1,.reload_config = 0 };
...
char *trim(char *str)
{
	return config_trim(str);
}

char *remove_quotes(char *str)
{
	return config_remove_quotes(str);
}

int load_config(const char *filename)
{
	return config_load(filename, &ctx.config);
}
...
int calculate_temperature(void)
{
	...
	if (hour >= ctx.config.sunset_hour
	    || hour < ctx.config.sunrise_hour) {
		return ctx.config.night_temp;
	}
	return ctx.config.day_temp;
}
...
{
	int effective_size =
	    (ctx.config.gamma_size >
	     0) ? ctx.config.gamma_size : gamma_size;
	...
	fill_gamma_luts(effective_size, temp, 1.0, red_lut, green_lut,
			blue_lut);
...
// Get CRTC info
int
get_crtc_info(int fd, uint32_t crtc_id, int *gamma_size, int *mode_valid)
{
...
static int connector_matches(int fd, uint32_t connector_id)
{
	...
	static const char *type_names[] = {
		...
		[5] = "Composite",[6] = "SVIDEO",[7] = "LVDS",[8] =
		    "Component",
		[9] = "9PinDIN",[10] = "DisplayPort",[11] = "HDMIA",[12] =
		    "HDMIB",
		[13] = "TV",[14] = "eDP",[15] = "VIRTUAL",[16] =
		    "DSI",[17] = "DPI",
	};
	...
	snprintf(name_long, sizeof(name_long), "%s-%u", tname,
		 conn.connector_type_id);
	/* Common short aliases */
	const char *alias = tname;
	if (conn.connector_type == 10)
		alias = "DP";
	else if (conn.connector_type == 11)
		alias = "HDMI-A";
	else if (conn.connector_type == 12)
		alias = "HDMI-B";
	else if (conn.connector_type == 14)
		alias = "eDP";
	snprintf(name_short, sizeof(name_short), "%s-%u", alias,
		 conn.connector_type_id);

	return (strcasecmp(ctx.config.connector, name_long) == 0 ||
		strcasecmp(ctx.config.connector, name_short) == 0);
}
...
static uint32_t get_matching_crtc_mask(int fd)
{
	...
	uint32_t *enc_ids =
	    calloc(res.count_encoders ? res.count_encoders : 1,
		   sizeof(uint32_t));
	uint32_t *crtc_ids = calloc(res.count_crtcs ? res.count_crtcs : 1,
				    sizeof(uint32_t));
	uint32_t *fb_ids =
	    calloc(res.count_fbs ? res.count_fbs : 1, sizeof(uint32_t));
	...
	for (uint32_t c = 0; c < res.count_connectors; c++) {
		...
		struct drm_mode_get_connector gc = {.connector_id =
			    conn_ids[c]
		};
		if (ioctl(fd, DRM_IOCTL_MODE_GETCONNECTOR, &gc) < 0
		    || gc.encoder_id == 0)
			continue;

		struct drm_mode_get_encoder enc = {.encoder_id =
			    gc.encoder_id
		};
...
static int
apply_temperature_to_fd(int fd, const char *device_name, int temp)
{
	struct drm_mode_card_res res = { 0 };
	if (ioctl(fd, DRM_IOCTL_MODE_GETRESOURCES, &res) < 0) {
		log_msg("ERROR", "Failed to get DRM resources for %s",
			device_name);
		return -1;
	}
	...
	uint32_t *fbs =
	    calloc(res.count_fbs ? res.count_fbs : 1, sizeof(uint32_t));
	uint32_t *connectors =
	    calloc(res.count_connectors ? res.count_connectors : 1,
		   sizeof(uint32_t));
	uint32_t *encoders =
	    calloc(res.count_encoders ? res.count_encoders : 1,
		   sizeof(uint32_t));
	...
	if (ioctl(fd, DRM_IOCTL_MODE_GETRESOURCES, &res) < 0) {
		log_msg("ERROR", "Failed to get CRTC list for %s",
			device_name);
		free(crtcs);
		free(fbs);
		free(connectors);
	...
	for (uint32_t i = 0; i < res.count_crtcs; i++) {
		...
		if (get_crtc_info(fd, crtc_id, &gamma_size, &mode_valid) <
		    0)
			continue;
...
int apply_temperature(int temp)
{
	...
	for (int d = 0; d < ctx.config.num_devices; d++) {
		char actual_device[256];
		int fd =
		    drm_open_device(ctx.config.devices[d], actual_device,
				    sizeof(actual_device));
		if (fd < 0) {
			log_msg("ERROR",
				"Failed to open DRM device %s: %s",
				ctx.config.devices[d], strerror(errno));
			continue;
		}
		if (ctx.config.verbose
		    && strcmp(actual_device, ctx.config.devices[d]) != 0) {
			log_msg("INFO", "Using device %s (requested %s)",
				actual_device, ctx.config.devices[d]);
		}
...
void daemon_loop(const char *config_file)
{
	...
	log_msg("INFO",
		"Monitoring TTY %d (auto), TTY %d (force warm), TTY %d (force cool)",
		ctx.config.monitor_tty, ctx.config.warm_tty,
		ctx.config.cool_tty);
	log_msg("INFO", "Day: %dK, Night: %dK", ctx.config.day_temp,
		ctx.config.night_temp);
	log_msg("INFO", "Sunset: %02d:00, Sunrise: %02d:00",
		ctx.config.sunset_hour, ctx.config.sunrise_hour);
	if (ctx.config.connector[0])
		log_msg("INFO", "Connector filter: %s",
			ctx.config.connector);
	if (ctx.config.gamma_size > 0)
		log_msg("INFO", "Gamma table size override: %d",
			ctx.config.gamma_size);
	...
	if (inotify_fd < 0) {
		log_msg("ERROR", "Failed to initialize inotify: %s",
			strerror(errno));
		return;
	}
	...
	if (watch_fd < 0) {
		log_msg("WARN", "Cannot watch config directory %s: %s",
			config_dir, strerror(errno));
		log_msg("INFO",
			"Config auto-reload disabled - changes require daemon restart");
	} else {
	...
	while (ctx.running) {
		...
		if (active_vt < 0) {
			sleep(ctx.config.check_interval);
			continue;
		}
		// Detect VT switch
		if (active_vt != prev_vt) {
			...
			if (active_vt == ctx.config.monitor_tty) {
				// Switch to monitor TTY: apply time-based temperature
				target_temp = calculate_temperature();
				log_msg("INFO",
					"User switched to TTY%d (auto)",
					ctx.config.monitor_tty);
			} else if (active_vt == ctx.config.warm_tty) {
				// Switch to warm TTY: force warm (night) temperature
				target_temp = ctx.config.night_temp;
				log_msg("INFO",
					"User switched to TTY%d (force warm: %dK)",
					ctx.config.warm_tty, target_temp);
			} else if (active_vt == ctx.config.cool_tty) {
				// Switch to cool TTY: force cool (day) temperature
				target_temp = ctx.config.day_temp;
				log_msg("INFO",
					"User switched to TTY%d (force cool: %dK)",
					ctx.config.cool_tty, target_temp);
			}

			if (target_temp > 0
			    && (target_temp != last_applied_temp
				|| last_applied_temp == 0)) {
				log_msg("INFO",
					"Applying temperature: %dK",
					target_temp);

				if (apply_temperature(target_temp) == 0) {
					log_msg("INFO",
						"Successfully applied %dK",
						target_temp);
					last_applied_temp = target_temp;
				} else {
					log_msg("ERROR",
						"Failed to apply temperature");
				}
			}
		...
		int is_on_target_tty =
		    (active_vt == ctx.config.monitor_tty);

		// Periodic check for time-based changes (every minute)
		time_t now = time(NULL);
		if (now - last_check >= 60) {
			int current_temp = calculate_temperature();
			if (current_temp != last_applied_temp
			    && is_on_target_tty) {
				log_msg("INFO",
					"Time-based temperature change: %dK",
					current_temp);
...
void print_usage(const char *prog)
{
	...
	printf
	    ("DRM Color Temperature TTY Daemon with inotify config watching\n\n");
	printf("Options:\n");
...
int main(int argc, char *argv[])
{
	...
	// Initialize syslog (must be after load_config so ctx.config.verbose is known)
	drm_log_init("drm_colortemp_daemon", ctx.config.verbose);
