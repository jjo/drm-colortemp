...
void
log_msg (const char *level, const char *format, ...)
{
  int prio = LOG_INFO;
...
calculate_temperature (void)
{
  ...
  if (hour >= ctx.config.sunset_hour || hour < ctx.config.sunrise_hour)
    {
...
int
set_gamma_temp (int fd, uint32_t crtc_id, int gamma_size, int temp)
{
  int effective_size =
    (ctx.config.gamma_size > 0) ? ctx.config.gamma_size : gamma_size;
...
static int
connector_matches (int fd, uint32_t connector_id)
{
...
get_matching_crtc_mask (int fd)
{
  ...
  uint32_t *enc_ids = calloc (res.count_encoders ? res.count_encoders : 1,
			      sizeof (uint32_t));
...
apply_temperature_to_fd (int fd, const char *device_name, int temp)
{
  ...
  uint32_t *connectors =
    calloc (res.count_connectors ? res.count_connectors : 1,
	    sizeof (uint32_t));
  uint32_t *encoders = calloc (res.count_encoders ? res.count_encoders : 1,
			       sizeof (uint32_t));
  ...
  for (int d = 0; d < ctx.config.num_devices; d++)
    {
      ...
      int fd = drm_open_device (ctx.config.devices[d], actual_device,
...
void
daemon_loop (const char *config_file)
{
  ...
  log_msg ("INFO",
	   ...
	   ctx.config.monitor_tty, ctx.config.warm_tty, ctx.config.cool_tty);
  ...
  if (ctx.config.connector[0])
    log_msg ("INFO", "Connector filter: %s", ctx.config.connector);
  if (ctx.config.gamma_size > 0)
    log_msg ("INFO", "Gamma table size override: %d", ctx.config.gamma_size);
  ...
  while (ctx.running)
      ...
      if (active_vt != prev_vt)
	{
	  ...
	  if (target_temp > 0
	      && (target_temp != last_applied_temp || last_applied_temp == 0))
	    {
      ...
      int is_on_target_tty = (active_vt == ctx.config.monitor_tty);
      ...
      if (now - last_check >= 60)
	{
	  ...
	  if (current_temp != last_applied_temp && is_on_target_tty)
	    {
	      log_msg ("INFO",
		       "Time-based temperature change: %dK", current_temp);
