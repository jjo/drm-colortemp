...
void
fill_gamma_luts(int gamma_size, int temp, double brightness,
		uint16_t * red, uint16_t * green, uint16_t * blue)
{
	...
	// Handle edge case: gamma_size < 2 would cause division by zero
	if (gamma_size < 2) {
		// Single entry: set to scaled value
		red[0] = (uint16_t) (r_mult * 65535.0);
		green[0] = (uint16_t) (g_mult * 65535.0);
		blue[0] = (uint16_t) (b_mult * 65535.0);
		return;
	}
...
bool safe_atoi(const char *str, int *out, int min, int max)
{
	...
	// Clear errno and parse
	errno = 0;
	val = strtol(str, &end, 10);

	// Check for overflow, underflow, or no conversion
