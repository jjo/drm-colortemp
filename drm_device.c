...
int
drm_device_accessible (const char *device)
{
  if (!device)
    {
      return 0;
...
int
drm_device_has_crtcs (const char *device)
{
  int fd = open (device, O_RDWR);
  if (fd < 0)
    {
...
int
drm_find_device (char *device_out, size_t device_out_size)
{
...
int
drm_find_all_devices (char (*devices)[256], int max_devices)
{
  int count = 0;
...
int
drm_open_device (const char *preferred_device, char *device_out,
		 size_t device_out_size)
{
  ...
  if (preferred_device && preferred_device[0] != '\0')
      ...
      if (drm_device_has_crtcs (preferred_device))
	{
	  ...
	  if (fd >= 0)
	    {
	      snprintf (actual_device, sizeof (actual_device), "%s",
			preferred_device);
	    }
	}
    }
