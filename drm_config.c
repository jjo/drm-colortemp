...
parse_devicen(const char *key, const char *value, config_t * cfg)
{
	...
	cfg->devices[idx][sizeof(cfg->devices[idx]) - 1] = '\0';
...
config_apply_kv(const char *key, const char *value, config_t * cfg)
{
	...
	for (const kv_entry_t * e = kv_table; e->key; e++) {
		...
		case KV_INT:
			if (safe_atoi
			    (value, &parsed_val, e->min_val, e->max_val)) {
			...
			break;
		case KV_STR:
			strncpy((char *) cfg + e->offset, value,
				e->size - 1);
			((char *) cfg + e->offset)[e->size - 1] = '\0';
			break;
		case KV_DEVICE:
			strncpy(cfg->devices[0], value,
				sizeof(cfg->devices[0]) - 1);
...
static void config_validate(config_t * cfg)
{
	if (cfg->day_temp < 1000 || cfg->day_temp > 10000) {
		DRM_LOG_WRN
		    ("DAY_TEMP %d out of range [1000,10000], clamping",
		     cfg->day_temp);
	...
	if (cfg->gamma_size != 0 &&
	    (cfg->gamma_size < GAMMA_SIZE_MIN
	     || cfg->gamma_size > GAMMA_SIZE_MAX)) {
		DRM_LOG_WRN
...
int config_load(const char *filename, config_t * cfg)
{
	...
	while (fgets(line, sizeof(line), f)) {
		...
		if (!equals) {
			DRM_LOG_WRN("config:%d: malformed line (no '=')",
				    line_num);
			continue;
		}
	...
	if (cfg->num_devices == 0) {
		int found =
		    drm_find_all_devices(cfg->devices, MAX_DEVICES);
		if (found > 0) {
			cfg->num_devices = found;
			DRM_LOG_INF("Auto-detected %d DRM device(s)",
				    found);
		} else {
			if (drm_find_device
			    (cfg->devices[0],
			     sizeof(cfg->devices[0])) == 0) {
				cfg->num_devices = 1;
			}
		}
	}
	...
	if (cfg->verbose) {
		...
		DRM_LOG_INF("Monitor TTY: %d, Warm TTY: %d, Cool TTY: %d",
			    cfg->monitor_tty, cfg->warm_tty,
			    cfg->cool_tty);
		if (cfg->has_location) {
			DRM_LOG_INF("Location: %.4f, %.4f", cfg->latitude,
				    cfg->longitude);
		}
		if (cfg->connector[0]) {
			DRM_LOG_INF("Connector filter: %s",
				    cfg->connector);
		}
		if (cfg->gamma_size > 0) {
			DRM_LOG_INF("Gamma table size override: %d",
				    cfg->gamma_size);
		}
	}
