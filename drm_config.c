...
typedef struct {
    const char *key;
    enum { KV_INT, KV_STR, KV_LOCATION, KV_DEVICE, KV_DEVICEN } kind;
...
static const kv_entry_t kv_table[] = {
    { "DEVICE",         KV_DEVICE,   0, 0 },
    { "DAY_TEMP",       KV_INT,      OFF(day_temp),       0 },
    { "NIGHT_TEMP",     KV_INT,      OFF(night_temp),     0 },
    { "SUNSET_HOUR",    KV_INT,      OFF(sunset_hour),    0 },
    { "SUNRISE_HOUR",   KV_INT,      OFF(sunrise_hour),   0 },
    { "MONITOR_TTY",    KV_INT,      OFF(monitor_tty),    0 },
    { "WARM_TTY",       KV_INT,      OFF(warm_tty),       0 },
    { "COOL_TTY",       KV_INT,      OFF(cool_tty),       0 },
    { "CHECK_INTERVAL", KV_INT,      OFF(check_interval), 0 },
    { "VERBOSE",        KV_INT,      OFF(verbose),        0 },
    { "LOCATION",       KV_LOCATION, 0, 0 },
    { "CONNECTOR",      KV_STR,      OFF(connector), sizeof(((config_t*)0)->connector) },
    { "GAMMA_SIZE",     KV_INT,      OFF(gamma_size),     0 },
    { NULL, 0, 0, 0 }
};

/* Try to match DEVICEn (n = 1..MAX_DEVICES) */
static int parse_devicen(const char *key, const char *value, config_t *cfg) {
    if (strncmp(key, "DEVICE", 6) != 0) return 0;
    char ch = key[6];
    if (ch < '1' || ch > '0' + MAX_DEVICES || key[7] != '\0') return 0;
    int idx = ch - '1';
    strncpy(cfg->devices[idx], value, sizeof(cfg->devices[idx]) - 1);
    if (cfg->num_devices < idx + 1) cfg->num_devices = idx + 1;
    return 1;
}

/* Apply a single key=value pair */
static void config_apply_kv(const char *key, const char *value, config_t *cfg) {
    ...
    for (const kv_entry_t *e = kv_table; e->key; e++) {
        ...
        switch (e->kind) {
        case KV_INT:
            *(int *)((char *)cfg + e->offset) = atoi(value);
            break;
        case KV_STR:
        ...
        case KV_DEVICE:
            ...
            if (cfg->num_devices < 1) cfg->num_devices = 1;
            break;
        case KV_LOCATION:
            if (strlen(value) > 0) {
                if (sscanf(value, "%lf,%lf", &cfg->latitude, &cfg->longitude) == 2) {
                    cfg->has_location = 1;
                }
            }
            break;
...
static void config_validate(config_t *cfg) {
    if (cfg->day_temp < 1000 || cfg->day_temp > 10000) {
        DRM_LOG_WRN("DAY_TEMP %d out of range [1000,10000], clamping", cfg->day_temp);
        if (cfg->day_temp < 1000) cfg->day_temp = 1000;
        if (cfg->day_temp > 10000) cfg->day_temp = 10000;
    }
    if (cfg->night_temp < 1000 || cfg->night_temp > 10000) {
        DRM_LOG_WRN("NIGHT_TEMP %d out of range [1000,10000], clamping", cfg->night_temp);
        if (cfg->night_temp < 1000) cfg->night_temp = 1000;
        if (cfg->night_temp > 10000) cfg->night_temp = 10000;
    }
    if (cfg->sunset_hour < 0 || cfg->sunset_hour > 23) {
        DRM_LOG_WRN("SUNSET_HOUR %d out of range [0,23], resetting to 20", cfg->sunset_hour);
        cfg->sunset_hour = 20;
    }
    if (cfg->sunrise_hour < 0 || cfg->sunrise_hour > 23) {
        DRM_LOG_WRN("SUNRISE_HOUR %d out of range [0,23], resetting to 8", cfg->sunrise_hour);
        cfg->sunrise_hour = 8;
    }
    ...
    if (cfg->gamma_size != 0 &&
        ...
        DRM_LOG_WRN("GAMMA_SIZE %d out of range [%d,%d], resetting to 0 (auto)",
...
int config_load(const char *filename, config_t *cfg) {
    ...
    if (cfg->num_devices == 0) {
        ...
        if (found > 0) {
            ...
            if (drm_find_device(cfg->devices[0], sizeof(cfg->devices[0])) == 0) {
                cfg->num_devices = 1;
            }
        }
    }
    ...
    if (cfg->verbose) {
        ...
        DRM_LOG_INF("Day temp: %dK, Night temp: %dK", cfg->day_temp, cfg->night_temp);
        DRM_LOG_INF("Sunset: %02d:00, Sunrise: %02d:00", cfg->sunset_hour, cfg->sunrise_hour);
